...
import "./DiscountDealsCarousel.css";
...
export default function DiscountDealsCarousel({ products = [] }) {
  ...
  return (
    <section className="discountDealsSection">
      <div className="discountDealsHeader">
        <h2>Top Deals</h2>
      </div>
      <div className="discountDealsCarousel">
        <button
          className="discountDealsArrow discountDealsArrow--left"
          onClick={goPrev}
          disabled={index === 0}
        ...
        <div className="discountDealsTrack">
          {currentProducts.length > 0 ? (
            currentProducts.map((product) => {
              ...
              return (
                <article
                  key={product.id}
                  className="discountDealCard"
                  onClick={() => handleClick(product)}
                >
                  <div className="discountDealImageWrapper">
                    <img src={getImageUrl(image)} alt={product.product_name} />
                    <span className="discountBadge">{discount.toFixed(0)}% off</span>
                    <span className="discountTag">Black Friday Deal</span>
                  </div>
                  <div className="discountDealPriceRow">
                    <div className="discountDealPrice">{formatCurrencyUtil(price)}</div>
                    <div className="discountDealOriginal">{formatCurrencyUtil(original)}</div>
                  </div>
                  <div className="discountDealName">{product.product_name}</div>
                </article>
              );
            })
          ) : (
            <div className="discountDealsEmpty">No deals available</div>
          )}
        </div>
        <button
          className="discountDealsArrow discountDealsArrow--right"
          onClick={goNext}
